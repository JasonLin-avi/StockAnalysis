 ...
 *
 * Why: Fetches and displays full-width AI technical diagnosis generated by LLM (Gemini)
...
function TechnicalAISummaryPanel({ symbol }) {
  ...
  useEffect(() => {
    let isMounted = true;
    const fetchAITechnicalSummary = async () => {
      if (!symbol) return;
      setIsLoading(true);
      setError(null);

      try {
        const response = await fetch(`/api/stock/${symbol}/technical-ai`);
        if (!response.ok) {
          const errJson = await response.json().catch(() => ({}));
