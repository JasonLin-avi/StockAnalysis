  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6">
        <div className="w-full flex flex-col md:flex-row items-center justify-around gap-4">
          ...
          <div className="w-full md:w-1/2 grid grid-cols-2 gap-3 text-xs">
            <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
              <div className="text-slate-400 mb-0.5 font-medium">市盈率 P/E</div>
              <div className="font-bold text-slate-100 text-sm">
                {pe?.value !== null ? `${pe?.value}x` : 'N/A'}
              </div>
              <div className={`text-[10px] mt-1 font-semibold ${
                pe?.status === 'Undervalued' ? 'text-emerald-400' :
                pe?.status === 'Fair' ? 'text-amber-400' : 'text-rose-400'
              }`}>
                {pe?.status}
              </div>
            </div>
            ...
            <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
              <div className="text-slate-400 mb-0.5 font-medium">每股收益 EPS</div>
              <div className="font-bold text-slate-100 text-sm">
                {eps?.value !== null ? `${eps?.value}` : 'N/A'}
              </div>
              <div className={`text-[10px] mt-1 font-semibold ${
                eps?.status === 'Strong' ? 'text-emerald-400' : 'text-slate-400'
              }`}>
            ...
            <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
              <div className="text-slate-400 mb-0.5 font-medium">負債比率 Debt</div>
              <div className="font-bold text-slate-100 text-sm">
                {debtRatio?.value !== null ? `${(debtRatio?.value * 100).toFixed(2)}%` : 'N/A'}
              </div>
              <div className={`text-[10px] mt-1 font-semibold ${
                debtRatio?.status === 'Healthy' ? 'text-emerald-400' :
                debtRatio?.status === 'Moderate' ? 'text-amber-400' : 'text-rose-400'
              }`}>
                {debtRatio?.status}
              </div>
            </div>
            ...
            <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
              <div className="text-slate-400 mb-0.5 font-medium">營收成長 Revenue</div>
              <div className="font-bold text-slate-100 text-sm">
                {revenueGrowth?.value !== null ? `${(revenueGrowth?.value * 100).toFixed(2)}%` : 'N/A'}
              </div>
              <div className={`text-[10px] mt-1 font-semibold ${
                revenueGrowth?.status === 'High Growth' ? 'text-emerald-400' :
                revenueGrowth?.status === 'Stable Growth' ? 'text-slate-400' : 'text-rose-400'
              }`}>
                {revenueGrowth?.status}
              </div>
            </div>

            <div className="col-span-2 bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
              <div className="text-slate-400 mb-0.5 font-medium">盈餘品質 Cash Flow (自由現金流佔比)</div>
              <div className="font-bold text-slate-100 text-sm">
                {cashFlow?.value !== undefined && cashFlow?.value !== null ? `${(cashFlow?.value * 100).toFixed(2)}%` : 'N/A'}
              </div>
              <div className={`text-[10px] mt-1 font-semibold ${
                cashFlow?.status === 'Healthy' ? 'text-emerald-400' :
                cashFlow?.status === 'Moderate' ? 'text-amber-400' : 'text-rose-400'
              }`}>
                {cashFlow?.status}
              </div>
            </div>
        ...
        <div className="pt-4 border-t border-slate-800/80 text-[11px] leading-relaxed">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-300 mb-3.5 flex items-center gap-1.5">
            📋 財務健康指標評估說明
          </h4>
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
              <div className="sm:w-1/4 font-semibold text-slate-300">市盈率 P/E (估值)</div>
              <div className="sm:w-3/4 text-slate-400">
                評估當前股價相較於盈餘的溢價程度。當前評為 <span className="text-slate-200 font-medium">{pe?.status || 'N/A'}</span> ({pe?.value !== null ? `${pe?.value}x` : 'N/A'})。
                基準為：小於 15x 為低估值，15~30x 為合理估值，大於 30x 為高溢價/高估。
              </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
              <div className="sm:w-1/4 font-semibold text-slate-300">每股收益 EPS (盈利能力)</div>
              <div className="sm:w-3/4 text-slate-400">
                公司經營獲取淨利的能力。當前評為 <span className="text-slate-200 font-medium">{eps?.status || 'N/A'}</span> (趨勢為 {eps?.trend || 'N/A'}，當前為 {eps?.value ?? 'N/A'})。
                正值且連年增長代表企業經營前景強勁，具有長期競爭優勢（護城河）。
              </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
              <div className="sm:w-1/4 font-semibold text-slate-300">負債比率 Debt (償債能力)</div>
              <div className="sm:w-3/4 text-slate-400">
                衡量公司的債務槓桿與財務安全性。當前評為 <span className="text-slate-200 font-medium">{debtRatio?.status || 'N/A'}</span> ({debtRatio?.value !== null ? `${(debtRatio?.value * 100).toFixed(2)}%` : 'N/A'})。
                小於 50% 為債務健康，50%~80% 屬合理負債，大於 80% 代表財務高槓桿、面臨高利息與財務重組風險。
              </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
              <div className="sm:w-1/4 font-semibold text-slate-300">營收成長 Revenue (擴張速度)</div>
              <div className="sm:w-3/4 text-slate-400">
                反映公司市場佔有率與銷售額擴張速率。當前評為 <span className="text-slate-200 font-medium">{revenueGrowth?.status || 'N/A'}</span> ({revenueGrowth?.value !== null ? `${(revenueGrowth?.value * 100).toFixed(2)}%` : 'N/A'})。
                成長率大於 15% 屬高擴張，0%~15% 為穩定擴張，低於 0% 代表業務出現萎縮。
              </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4">
              <div className="sm:w-1/4 font-semibold text-slate-300">盈餘品質 Cash Flow (盈餘品質)</div>
              <div className="sm:w-3/4 text-slate-400">
                衡量帳面利潤是否能實質轉化為現金。當前評為 <span className="text-slate-200 font-medium">{cashFlow?.status || 'N/A'}</span> ({cashFlow?.value !== null ? `${(cashFlow?.value * 100).toFixed(2)}%` : 'N/A'})。
