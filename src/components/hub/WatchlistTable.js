...
export default function WatchlistTable() {
  ...
  const [timeframe, setTimeframe] = useState('5d'); // '5d', '10d', '20d'
  ...
  // Why: Dynamic multi-column sorting over the watchlist array using price data and timeframe-specific backtest metrics.
  const sortedWatchlist = [...watchlist].sort((a, b) => {
    ...
    const winRateKey = `winRate${timeframe}`;
    const avgReturnKey = `avgReturn${timeframe}`;
    ...
    if (sortField === 'symbol') {
      ...
      valA = itemA[winRateKey] || 0;
      valB = itemB[winRateKey] || 0;
    } else if (sortField === 'avgReturn') {
      valA = itemA[avgReturnKey] || 0;
      valB = itemB[avgReturnKey] || 0;
    }
  ...
  const timeframeLabel = timeframe === '5d' ? '5日' : (timeframe === '10d' ? '10日' : '20日');

  return (
    <div className="space-y-4">
      {/* Timeframe Filter Bar */}
      <div className="flex items-center justify-between bg-[#0B0F19]/60 border border-slate-800/80 p-3 rounded-xl backdrop-blur">
        <div className="flex items-center gap-2 text-xs font-mono text-slate-400">
          <span className="text-slate-500">回測持有期 (Holding Period):</span>
          {[
            { id: '5d', label: '5D (極短線)' },
            { id: '10d', label: '10D (波段)' },
            { id: '20d', label: '20D (中線)' }
          ...
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {sortedWatchlist.map(symbol => {
                ...
                const winRateVal = quote[`winRate${timeframe}`] || 0;
                const avgReturnVal = quote[`avgReturn${timeframe}`] || 0;

                const winRatePct = (winRateVal * 100).toFixed(0);
