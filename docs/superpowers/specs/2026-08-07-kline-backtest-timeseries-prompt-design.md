...
2. **Full Time-Series K-Line Prompt (15-Year Senior Quant Persona)**: The `/api/backtest/predict` prompt will render a complete daily time-series Markdown table containing OHLCV, MA5, MA20, MA60, RSI(14), and MACD values for every day in the lookback window. This enables true pattern recognition (e.g. W-bottoms, breakout bars, volume confirmation) over continuous lines.
...
### 3.1 Time-Series Data Assembly

The backend retrieves `lookbackDays` trading days prior to `cutoffDate`, computes moving averages (MA5/20/60), RSI(14), and MACD for each bar, and formats them into a Markdown table:
...
|---|---|---|---|---|---|---|---|---|---|---|
... (Daily records for all lookbackDays leading up to cutoffDate)
```

### 3.2 15-Year Quant Trader Persona Prompt

```markdown
# Role (角色設定)
你是一位擁有 15 年經驗的資深量化交易員與資產配置專家。你的分析風格兼顧宏觀趨勢與微觀進出，重視波段型態、價量配合與短中線風險報酬比（Risk/Reward Ratio）。
...
# Task (分析任務)
請站在 ${cutoffDate} 的當下視角，完整檢視過去 ${lookbackDays} 個交易日的 K 線波動軌跡、K 線型態 (如 W 底/頭肩頂/帶量突破/支撐扣抵)、價量關係與指標變化。
